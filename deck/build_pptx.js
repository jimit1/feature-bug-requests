...
const notes = {
  ...
  7:'Hallucination. Six checks in the order the data meets them: exact quotes with a locator, a fixed schema so free text is impossible, a code check that the quote is in the cited source, a stated reason on every editor decision, an ask agent that knows only the library and says when it does not cover a question, and a golden set every night, covering how the editor files as well as the claims, that fails the job on drift.',
  8:'Scope. Built only what the brief asked for. A wider scrub, live connectors and an approve gate are drawn on the system; a database, sign-in with memory, status per theme, weekly notes, more sources, per-account and trend views are listed for later. None of it is built.',
